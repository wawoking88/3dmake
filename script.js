...
function setEditorLayerDeep(obj){
  obj.layers.set(EDITOR_LAYER);
  if(obj.traverse){
    obj.traverse(child => child.layers.set(EDITOR_LAYER));
  }
}
...
function makeArrowAxis(name, color, axis){
  const group = new THREE.Group();
  group.name = name;

  const lineMat = new THREE.LineBasicMaterial({color, depthTest:false});
  const meshMat = new THREE.MeshBasicMaterial({color, depthTest:false});

  let end = new THREE.Vector3(1.8,0,0);
  if(axis === "y") end.set(0,1.8,0);
  if(axis === "z") end.set(0,0,1.8);
  ...
  const cone = new THREE.Mesh(new THREE.ConeGeometry(0.14,0.38,24), meshMat);
  cone.name = name;

  if(axis === "x"){
    cone.position.set(2.0,0,0);
...
function makeScaleAxis(name, color, axis){
  const group = new THREE.Group();
  group.name = name;
  ...
  let end = new THREE.Vector3(1.65,0,0);
  if(axis === "y") end.set(0,1.65,0);
  if(axis === "z") end.set(0,0,1.65);
  ...
  const cube = new THREE.Mesh(new THREE.BoxGeometry(0.28,0.28,0.28), meshMat);
  cube.name = name;
  cube.position.copy(end);
  group.add(cube);
...
function makeRotateRing(name, color, axis){
  const group = new THREE.Group();
  group.name = name;
  ...
  for(let i=0;i<=128;i++){
    ...
    if(axis === "x"){
      points.push(new THREE.Vector3(0, Math.cos(a)*r, Math.sin(a)*r));
    } else if(axis === "y"){
  ...
  const knobMat = new THREE.MeshBasicMaterial({color, depthTest:false});
  const knob = new THREE.Mesh(new THREE.SphereGeometry(0.14,16,12), knobMat);
  knob.name = name;

  if(axis === "x") knob.position.set(0,r,0);
  if(axis === "y") knob.position.set(r,0,0);
  if(axis === "z") knob.position.set(r,0,0);
...
function clearGizmo(){
  while(gizmo.children.length){
    const child = gizmo.children.pop();
...
function rebuildGizmo(){
  ...
  if(editMode === "translate"){
    gizmo.add(
      makeArrowAxis("x",0xff3333,"x"),
      makeArrowAxis("y",0x33cc33,"y"),
      makeArrowAxis("z",0x3366ff,"z")
    );
  } else if(editMode === "scale"){
    gizmo.add(
      makeScaleAxis("x",0xff3333,"x"),
      makeScaleAxis("y",0x33cc33,"y"),
      makeScaleAxis("z",0x3366ff,"z")
    );
  } else if(editMode === "rotate"){
    gizmo.add(
      makeRotateRing("x",0xff3333,"x"),
      makeRotateRing("y",0x33cc33,"y"),
      makeRotateRing("z",0x3366ff,"z")
    );
  }
}
...
function updateGizmo(){
  ...
  const dist = previewCamera.position.distanceTo(s.object.position);
  const scale = Math.max(0.8, Math.min(2.2, dist / 8));
...
stageWrap.addEventListener("pointerdown", e=>{
  ...
  const gizHits = raycaster.intersectObjects(gizmo.children, true);
...
function execOne(s,b,dt){
  ...
  if(b.type==="set_scale"){ const sc=Math.max(.1,Number(b.fields?.SCALE??1)); s.object.scale.setScalar(sc); }
